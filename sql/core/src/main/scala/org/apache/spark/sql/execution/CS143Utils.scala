    ...
    new Iterator[Row] {
      ...
      def next() = {
        if (hasNext) {
          ...
          val preRes = preUdfProjection.apply(row)
          val postRes = postUdfProjection.apply(row)
          if (!cache.containsKey(key)) {
            cache.put(key, udfProject.apply(row))
          }
          val udfRes = cache.get(key)
          Row.fromSeq(preRes ++ udfRes ++ postRes)
        } else {
          null
        }
      }
...
object AggregateIteratorGenerator {
  def apply(resultExpressions: Seq[Expression],
    ...
    new Iterator[Row] {
      ...
      def next() = {
        val (current_group, current_table) = input.next()
        val result = new GenericMutableRow(1)
        result(0) = current_table.eval()

        postAggregateProjection(new JoinedRow(result, current_group))
      }
    }
  }
}
